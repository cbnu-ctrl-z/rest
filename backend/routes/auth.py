...
def signup():
    ...
    if not name or not id or not password: #세개중 하나라도 값이 없으면 에러메세지 반환환
    ...
    user_data = {'name': name, 'id': id, 'password': password, 'freetimes': []} #딕셔너리 형태로 user_data에 저장
...
def login():
    ...
    if not id or not password: #아무것도 입력안하면 에러메세지 반환환
